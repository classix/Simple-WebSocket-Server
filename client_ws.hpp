            ...
            void send_from_queue() {
                strand.post([this]() {
                    boost::asio::async_write(*socket, send_queue.begin()->send_stream->streambuf,
                        ...
                        auto send_callback = std::move(send_queued->callback); //hold it in local variable, because the closure might hold crucial resources (like this connection) and erase() would fail.
                        if(send_callback)
                            send_callback(ec);
                        if(!ec) {
